...
class MeasurementsData:
    def __init__(self) -> None:
        ...
        self.coincidences_count_rate_data: dict[
            str, dict[tuple[int, int], list[float]]
        ] = {}

    def update_single_counts_rate(
        self, device_serial: str, data_per_channel: dict[int, float]
    ):
        for channel in data_per_channel:
            self.single_counts_rate_data[device_serial][channel].append(
                data_per_channel[channel]
            )
